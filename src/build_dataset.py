...
def brw_at_points(year: int, points: gpd.GeoSeries) -> pd.Series:
    """Bodenrichtwert eines Stichtags an vorgegebenen Punkten auslesen."""
    if year == 2026:
        df = gpd.read_parquet(DATA_RAW / "boris_2026.parquet")
        # bereits über die 2024er-Zonen-ID verknüpft
        return df.set_index("gml_id_2024")["bodenrichtwert"]

    zones = gpd.read_parquet(DATA_RAW / f"boris_{year}.parquet").to_crs(CRS_METRIC)
    zones = zones[["bodenrichtwert", "art", "entwicklungszustand", "geometry"]]
    ...
    # Bei überlappenden Zonen die erste Zuordnung behalten
    joined = joined[~joined.index.duplicated(keep="first")]
    return joined["bodenrichtwert"]
...
def main() -> None:
    ...
    for year in stichtage:
        ...
        brw = brw_at_points(year, pts)
        if year == 2026:
            werte = ref["gml_id"].map(brw)
        else:
            werte = pd.Series(brw.values, index=ref.index)
        ...
        frame = pd.DataFrame(
            {
                ...
                "stichtag": year,
                "laermjahr": laermjahr,
                "bodenrichtwert": pd.to_numeric(werte, errors="coerce").values,
        ...
        print(f"  {year}: {n_ok} Werte zugeordnet, {n_laerm} Zonen mit Fluglärmpegel")
        frames.append(frame)
